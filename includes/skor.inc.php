...
class Skor
{
	...
	{
		$query = "INSERT INTO analisa_alternatif VALUES('$a','$b','','$c','$d')";
	...
	{
		$query = "UPDATE analisa_alternatif SET hasil_analisa_alternatif = '$a' WHERE alternatif_pertama = '$b' AND alternatif_kedua = '$c' AND id_kriteria='$d'";
		$stmt = $this->conn->prepare($query);
	...
	{
		$query = "SELECT sum(hasil_analisa_alternatif) AS jumkr2 FROM analisa_alternatif WHERE alternatif_kedua='$a' AND id_kriteria='$b'";
		$stmt = $this->conn->prepare($query);
		$stmt->execute();
		$row = $stmt->fetch(PDO::FETCH_ASSOC);
		$this->nak = $row['jumkr2'];
